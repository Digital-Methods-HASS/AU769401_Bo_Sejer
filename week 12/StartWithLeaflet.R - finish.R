...
leaflet() %>%
  ...
  setView(lng = -2.292279 , lat = 53.463493, zoom = 16)              # set the location of the map 
...
leaflet() %>% 
  addTiles() %>% 
  setView( lng = 11, lat = 56, zoom = 6 ) %>%  # let's use setView to navigate to our area
...
saveWidget(fortetkort, "fortetkort.html", selfcontained = TRUE)
...
I used chat gtp to get the code and clustering the answers together
...
Clustering makes it easier to comprehend the data, but is less specific in the overall picture.
