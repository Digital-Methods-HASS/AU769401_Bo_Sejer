---
title: "Danish Kings - Week10 "
author: "Sejer Bo Christensen"
date: "2025-05-03"
...
kings1 <- read.csv("data/kings.csv",na=c("NULL", ""))

kings2 <- read_csv("data/kings.csv",na=c("NULL", ""))

kings3 <- read.csv2("data/kings.csv",,na=c("NULL", ""))

kings4 <- read_csv2( "data/kings.csv",na=c("NULL", ""))
head(kings1)
glimpse(kings2)
head(kings3)
glimpse(kings4)
...
long_reigning_kings_count <- kings2 %>%
  mutate(Duration = End_year - Start_year) %>%
  filter(Duration > average_duration) %>%
  count()
...
"Jeg har brugt ChatGPT til at optimere denne opgave. - Blandt andet til (print) og (average_duration)"
```
...
kings2 %>%
  ...
  arrange(desc(Duration)) %>%
  slice(1:3) %>%
  mutate(Days = Duration * 365 + floor(Duration / 4)) %>%  # Tager højde for skudår (groft anslået)
  select(Name, Start_year, End_year, Duration, Days) %>%
  print()

"Kommandoen:(Desc), har jeg fundet ved hjælp af ChatGPT. I følge Kongehusets hjemmeside blev CHR.IV først kronet i 1596. Og altså ikke 1588. Det ville ændre resultatet, og det kan man se med et andet datasæt."
```
...
#ChatGPT har hjulpet med farverne (darkblue) og (red)"
```
